...
const getPaymentStatusInfo = (item) => {
  ...
  if (item.ownership_status === 'preorder') {
    ...
    if (daysRemaining === null) {
      return { status: '💭 空气', className: 'status-badge-preorder', showActions: true };
    }
          ...
          <div className="payment-info-section">
            ...
            {paymentInfo.showActions && (
              <div className="payment-actions">
                {(item.payment_status || 'deposit_only') === 'deposit_only' && (
                  <button 
                ...
                {daysRemaining !== null && daysRemaining < 0 && (item.payment_status || 'deposit_only') !== 'full_paid' && (
...
const WardrobeCategory = ({ category, categoryName }) => {
  ...
  const handleStatusSelect = (status) => {
    setFormData(prev => ({ ...prev, ownership_status: status }));
    setShowAddForm(true);
    ...
    <div className="wardrobe-category">
        ...
        <div className="add-button-container">
          <button 
            ...
            onClick={editingItem ? handleCancel : () => {
              if (showAddForm || showStatusSelect) {
                handleCancel();
              } else {
          ...
          >
            {editingItem ? '取消编辑' : (showAddForm || showStatusSelect ? '取消' : '添加服饰')}
          </button>
      ...
      {showAddForm && (
        <form onSubmit={handleSubmit} className="add-form">
          <h3>{editingItem ? `编辑${categoryName}` : `添加${categoryName}`}</h3>
          ...
          {formData.ownership_status === 'preorder' && (
            <div className="form-row">
              <label className="form-label">付款状态</label>
              <select
                value={formData.payment_status}
                onChange={(e) => setFormData({...formData, payment_status: e.target.value})}
              >
                <option value="deposit_only">仅付定金</option>
                <option value="full_paid">已付全款</option>
              </select>
            </div>
          )}
            ...
            <div className="preorder-fields">
              ...
              <div className="form-row">
                <input
                  ...
                  onChange={(e) => setFormData({...formData, final_payment: e.target.value})}
                />
                <input
                  type="date"
                  placeholder="尾款日期"
          ...
          <div className="sizes-selector">
            <label className="form-label">适用尺寸（可多选）</label>
          ...
          <div className="image-upload-container">
            ...
            <ImageUpload
              onImageSelect={(imageUrl) => setFormData({...formData, profile_image_url: imageUrl})}
